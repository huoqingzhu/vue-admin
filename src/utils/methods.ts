...
function debounce(callBack: Function, time: number) {
    let timer: number | undefined // 定时器的返回值 初始为null
    return () => {
 ...
 * @returns
 */
export const hex = (x: string) => ('0' + parseInt(x).toString(16)).slice(-2)
/**
 * RGB颜色转为十六进制颜色
 * @param rgb
 * @returns
...
export const rgbaToHex = (
    ...
    return {
        hex: '#' + hex(result[1]) + hex(result[2]) + hex(result[3]),
        r,
        g,
        b,
        a,
 ...
 * @param value string
 */
export const isHex = (value: string): boolean => (value.indexOf('#') !== -1)
 ...
 * @returns
 */
export const HEX2DEC = (hex: string) => parseInt(hex, 16).toString()
/**
 * 十六进制颜色转化为RGB颜色
 * @param hex string
 * @returns
...
export const HEX2RGB = (hex: string): { r: number; g: number; b: number } => {
    ...
    return {
        r: Number(HEX2DEC(hex.substring(0, 2))),
        g: Number(HEX2DEC(hex.substring(2, 4))),
        b: Number(HEX2DEC(hex.substring(4))),
    }
}
/**
 *
...
export const section = (
    ...
    return {
        r: startColor[0] + Math.floor(sR * step),
        g: startColor[1] + Math.floor(sG * step),
        b: startColor[2] + Math.floor(sB * step),
    }
}
/**
 * 判断类型
 ...
 * @returns number[]
 */
const makeNumber = (list: any[]): number[] => list.map((item) => Math.floor(Number(item) * 100) / 100)
/**
 * 根据文件名获取文件类型
 * @param file_path
 * @returns
