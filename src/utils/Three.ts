...
class Three {
   ...
   * @param lizi  是否开启粒子效果
   */
  constructor(container: HTMLElement, camera = true, Light = true) {
  ...
  // 监控点击事件
  onMouseMove = (
    value: { name: string; x: number; y: number },
    spriteData: Object3D[]
  ) => {
  ...
  // 获取屏幕坐标系
  wordPosToScreen(
    object: { updateMatrixWorld: () => void; matrixWorld: Matrix4 },
    camera: Camera
  ): { x: number; y: number } {
